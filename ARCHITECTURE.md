...
more and true for one that takes none, and only a step reads it.
...
| live | a bind that was refused | unchanged | nothing was bound, so nothing changes |
...
| `caller-lists-are-walked-by-hand` | No rustler list decoder ever touches a caller's term: every list a NIF takes (parameters on query, execute, query_with_changes, stream_open, stmt_bind and explain_analyze, the cancel-token lists on the cancellable NIFs, the authorizer's denied actions) is walked once by util.rs's own walker over enif_get_list_cell, which refuses a non-list tail with a structured error instead of the panic rustler's ListIterator raises; the walked cells feed both the first-element keyword rule and the element decode; an improper list therefore answers {:expected_list, %{reason, value_type}} (or the keyword twin) on every door and never aborts the VM. lib/xqlite.ex:params_count/1, the telemetry count, walks a parameter list by hand and stops at a tail that is not a list so the NIF names the tail. A token list's refusal is re-tagged {:invalid_cancel_tokens, …} by cancel.rs:decode_tokens. The :type_extensions option is walked by hand on every Xqlite door that takes it (nil and an absent option mean none; a proper list of extension modules passes — an atom naming a module that Code.ensure_loaded?/1 finds (the check loads it), that declares @behaviour Xqlite.TypeExtension and that exports both encode/1 and decode/1, on every door whether or not it decodes; a module that passed is remembered for the life of the node in a persistent term keyed {Xqlite.TypeExtension, module}, a refused one is asked again on the next call — and an atom that is no such module answers {:invalid_type_extensions, %{reason: :bad_element, position, value_type: :atom}} at its position, as any other element that does not belong; anything that is no proper list answers {:invalid_type_extensions, list_refusal}; all of it before any metadata or NIF call). | `util.rs:walk_list`, `util.rs:walk_params` | `improper_list_law_test.exs: "every list-taking door refuses an improper list", "the anchor: a stream refuses an improper parameter list at open", "the anchor: a proper list of the same values still streams"; test/type_extension_test.exs: "a term that is no list at all is refused", "a list whose tail is not one is refused", "an element that is no module name is refused by its position", "nil and an absent option both mean no extensions", "every door that takes the option judges it the same way", "an atom that names no module is refused by its position", "the first element that is no extension is the one reported", "nil and false as elements are refused, unlike the option's own nil", "a module with both callbacks but no declaration is refused", "a module that declares the behaviour but has no decode/1 is refused", "a module that passed is remembered for the node, a refused one is not"` |
...
| `statement-steps-only-when-bound` | A prepared statement with one or more parameters refuses to step until a bind has succeeded since prepare: the resource carries a flag, false at prepare for such a statement (true for one with none), set true by a successful bind and by clear_bindings (the one explicit way to run with NULLs), left unchanged by a refused bind (an earlier successful bind stays in force) and by reset (SQLite keeps bindings across a reset); stmt_step, stmt_multi_step and its cancellable twin answer {:parameters_unbound, %{expected: n}} for a false flag, behind the finalized and closed checks; expected is read once at prepare with sqlite3_bind_parameter_count. SQLite itself runs an unbound parameter as NULL. | `nif.rs:stmt_prepare`, `nif.rs:stmt_bind`, `statement.rs:XqliteStatement` | `nif/statement_test.exs: "stepping before any bind is refused", "a refused bind leaves nothing to run", "a closed connection is reported before an unbound statement"; test/bind_before_step_law_test.exs: "the anchor: a statement that takes a parameter is refused", "the anchor: a statement that takes none runs at once", "clear_bindings is how a caller asks for NULLs", "a refused bind leaves an earlier successful one in force", "an UPDATE refused at the step writes nothing"` |
