...
| `error-shapes-per-class` | Each class has one Elixir shape: bare atoms for lifecycle and input errors; `{tag, extended_code, message}` for busy/locked, read-only, schema-changed and authorization-denied; `{:constraint_violation, kind, details}`; the fallback `{:sqlite_failure, code, extended_code, message}`; `{:sql_input_error, %{code, message, sql, offset}}` when a `SqlInputError` would otherwise classify generically; and `{:busy_timeout_write_refused, %{policy, observers}}` for the one denial xqlite's own authorizer rule raises. | `error.rs` `impl Encoder for XqliteError` | `nif/authorizer_test.exs: "denying :delete blocks DELETE but leaves SELECT working"` |
...
| `type-extension-chain-first-match-wins` | `encode/1` answers `{:ok, value}`, `:skip` or `{:error, reason}` and `decode/1` answers `{:ok, value}` or `:skip`; the chain runs in list order, the first `{:ok, _}` or `{:error, _}` wins and the rest are not consulted, unmatched values pass through unchanged. `encode_value/2` answers `{:ok, value}` or `{:error, %{extension, reason}}`; `encode_params/2` answers `{:ok, params}` or `{:error, {:type_extension_refused, %{position, extension, reason}}}` with the 1-based parameter position. Every `Xqlite` function that takes parameters runs the chain on them — `query/4`, `execute/4`, `stream/4`, `bind/3`, `explain_analyze/4`, `query_cancellable/5`, `execute_cancellable/5`, `query_with_changes_cancellable/5` — and every one that returns a result decodes its rows. The refusal is reported inside the function's telemetry span. | `type_extension.ex:encode_value/2`, `encode_params/2` and `decode_value/2` | `type_extension_test.exs: "first matching extension wins"`, `"a refusal stops the chain and names the extension"`, `type_extension_query_test.exs: "a value written through execute/4 is found through all five"` |
