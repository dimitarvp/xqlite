...
| live | `stmt_multi_step` errors after reading rows | live, error held | hand back the rows with `done: false`, keep the error in `XqliteStatement.pending_error`; the next `multi_step` answers it, `reset` and `finalize` drop it |
...
| `nif-panics-unwind-and-are-caught` | The crate is built with panic = unwind, pinned in its own .cargo/config.toml for release and dev because a config file outranks the manifest and a machine-wide panic = abort would silently disarm rustler's guard; with unwinding, rustler catches a panic in a NIF body and in argument decoding and raises :nif_panicked in the calling process, the VM stays up, and a panic while a connection lock is held still poisons that lock for good; scripts/panic_strategy.exs checks the undefined symbols of the libraries named on its command line for _Unwind_RaiseException, mix verify hands it priv/native/xqlitenif.{so,dll} (the file both mix environments load) and the step fails closed when no symbol reader exists (nm, llvm-nm, or the tool named in XQLITE_SYMBOL_TOOL); the release workflow reads each built library's symbols into its job summary, report-only until the marker per target family is measured. | `native/xqlitenif/.cargo/config.toml`, `scripts/panic_strategy.exs` | `panic_strategy_script_test.exs: "the library the virtual machine loaded passes, and is named", "a symbol tool that is not on the machine fails the check"` |
...
| `nil-means-no-parameters` | A parameter term of nil means no parameters on every raw door that takes them (query, execute, query_with_changes, stream_open, stmt_bind, explain_analyze): util.rs:walk_params answers Params::Empty for nil, the one producer, so the doors cannot disagree; Xqlite.bind/2,3 guards is_list/1, so nil raises at the typed door. | `util.rs:walk_params` | `improper_list_law_test.exs: "nil binds nothing on every parameter door", "nil leaves the parameter of a one-parameter statement NULL"` |
| `statement-value-error-reports-once` | A value SQLite hands back that xqlite cannot read (invalid UTF-8 in a TEXT column) is reported once, for a row SQLite has already stepped past, which is never delivered: step/1 reports it at once; stmt_multi_step and its cancellable twin first hand back the rows read before it in the same batch with done: false and hold the error in the statement's pending-error slot (a Mutex<Option<XqliteError>> written and taken with the connection Mutex held, poison recovered), which the next multi_step call answers and clears, reset and finalize clear, and step/1 does not consult; when no row was read before it in the batch the error is answered at once; a cancellation discards its batch; every door then continues at the row after the bad one; the stream twin holds its error the same way and is finished after reporting it. | `nif.rs:stmt_multi_step_impl`, `statement.rs:XqliteStatement::store_pending_error` | `statement_value_error_test.exs: "the rows read before the bad one come back before the error", "a reset drops the error that was held back"; test/nif/stream_test.exs: "isolated: a stream that errored delivers no row after the bad one"` |
