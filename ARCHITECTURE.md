...
| `close-finalizes-children` | close finalizes every prepared statement, stream and blob still open on the connection under the Mutex, then frees the SQLite handle (a WAL database's -wal file is gone after close); a child's later operation answers :connection_closed and its finalize or close is :ok; a second close is :ok; sessions are not covered. | `connection.rs:close_connection` | `nif/connection_test.exs: "an unstepped prepared statement does not keep the handle open"` |
...
| `telemetry-events-have-one-catalogue` | Xqlite.Telemetry.events/0 is the event catalogue (35 entries, 14 of them spans): a source census test proves every emit and span literal in lib/ is in it, drift tests prove the moduledoc and the guide list exactly it, and a reachability test proves every entry fires. | `xqlite/telemetry.ex:events/0` | `telemetry_test.exs: "every emission site in lib/ spells out a catalogued event"` |
| `poisoned-connection-lock-is-never-recovered` | A poisoned connection Mutex is never recovered: every conn.lock() maps the poison error to LockError and the connection is abandoned (close included), so a thread that observes the poison is the only one that can ever be inside SQLite on that connection again — which is what lets stream_fetch finalize a statement while holding the poisoned lock. into_inner() appears only on the process-global log registry lock, the per-session guard and the stream's pending-error slot, never on a connection lock. | `connection.rs:with_conn`, `connection.rs:with_conn_mut` | unpinned |
...
| `pragma-accessors-refuse-unknown-names` | Every pragma door — Xqlite.Pragma.get/3,4, Xqlite.Pragma.put/4, Xqlite.get_pragma/2 and Xqlite.set_pragma/3 — resolves a name through one function, resolve_name/1, which folds ASCII case for atoms and strings alike; a name outside the typed schema is refused with {:error, {:unknown_pragma, name}} carrying the caller's own atom or string, before any PRAGMA statement is built (SQLite would otherwise parse and ignore the unknown pragma and report success), and a key that is neither an atom nor a string with {:invalid_pragma_name, key}; Xqlite.set_pragma/3 keeps the raw path for a name outside the spec. | `xqlite/pragma.ex:do_put/4`, `xqlite/pragma.ex:do_get_with_arg/4`, `xqlite/pragma.ex:resolve_name/1`, `xqlite/pragma.ex:do_get_no_arg/3` | `pragma_test.exs: "an unknown name is refused before any statement reaches SQLite"` |
