...
| `cancel-tokens-validated-before-the-nif` | A :cancel_tokens value — one token or a list — is validated before any cancellable NIF runs: Xqlite.validate_cancel_tokens/1 asks the NIF is_cancel_token/1, which takes the term undecoded and answers true only for a token from create_cancel_token/0, so every Xqlite door that takes tokens (query_cancellable/5, execute_cancellable/5, execute_batch_cancellable/3, query_with_changes_cancellable/5, multi_step_cancellable/3, backup_with_progress/6, stream/4, cancel_operation/1) answers {:error, {:invalid_cancel_tokens, value}} with the caller's value unchanged, inside its telemetry span, instead of raising; a list whose tail is not [] is refused the same way by a hand-written walk (all_cancel_tokens?/1) instead of raising from Enum. The raw XqliteNIF functions answer instead of raising for the token list itself — a term that is no list is {:expected_list, %{reason: :not_a_list, value_type}}, a broken tail the :improper_tail twin, and an element that is no live token {:invalid_cancel_tokens, %{position: n}}, one-based — while a raw stub still raises for a wrongly typed argument that is not a list (cancel_operation/1 for instance); nil is refused (pass [] for no tokens). | `xqlite.ex:validate_cancel_tokens/1`, `nif.rs:is_cancel_token` | `cancel_token_law_test.exs: "the anchor: a stream refuses a reference that is not a token", "the anchor: a token list that does not end in [] is refused"` |
...
| `nif-panics-unwind-and-are-caught` | The crate is built with panic = unwind, pinned in its own .cargo/config.toml for release and dev because a config file outranks the manifest and a machine-wide panic = abort would silently disarm rustler's guard; with unwinding, rustler catches a panic in a NIF body and in argument decoding and raises :nif_panicked in the calling process, the VM stays up, and a panic while a connection lock is held still poisons that lock for good; scripts/panic_strategy.exs checks the built library's undefined symbols for _Unwind_RaiseException as a mix verify step. | `native/xqlitenif/.cargo/config.toml`, `scripts/panic_strategy.exs` | unpinned |
| `elixir-list-doors-walk-by-hand` | An Elixir door that walks a caller's list walks it with its own recursive function whose last clause takes a tail that is not a list, so a list ending in something other than [] comes back as the door's own structured refusal — {:invalid_open_option, %{key: nil, reason: :not_a_pair, value: tail}} for open options and {:invalid_cancel_tokens, list} for cancel tokens — instead of raising from Enum. | `xqlite.ex:open_opts_fault/2`, `xqlite.ex:all_cancel_tokens?/1` | `bad_input_answers_test.exs: "an options list that does not end in [] is an answer"; test/cancel_token_law_test.exs: "the anchor: a token list that does not end in [] is refused"` |
| `nil-means-no-parameters-except-on-bind` | A parameter term of nil means no parameters on query, execute, query_with_changes, stream_open and explain_analyze, each checking for nil before walk_params, but not on stmt_bind, which answers {:expected_list, %{reason: :not_a_list, value_type: :atom}} for it; Xqlite.bind/2,3 guards is_list/1 so nil raises at the typed door. | `query.rs:core_query`, `query.rs:core_execute`, `nif.rs:bind_stream_params`, `explain_analyze.rs:bind_params` | unpinned |
