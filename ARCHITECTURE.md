...
| `positional-parameter-count-checked-on-every-door` | A positional parameter list whose length is not the statement's own parameter count is refused with {:invalid_parameter_count, %{expected, provided}} before any value is bound, on every door: rusqlite's checked binding for query, execute and query_with_changes and their cancellable twins, and stream.rs:require_parameter_count for the raw-FFI binds of stmt_bind, stream_open and explain_analyze, called for the empty list and nil (provided 0) as for a list — SQLite itself refuses neither shape, an unbound parameter reading as NULL. Named parameters keep SQLite's rule: a name the statement lacks is {:invalid_parameter_name, _}, a name left out stays NULL. For a list two or more elements too long the provided count differs by door family: the raw-FFI doors report the list's length, rusqlite's doors the first index the statement lacks. | `stream.rs:require_parameter_count` | `parameter_count_law_test.exs: "the anchor: a list one element short through a stream writes nothing", "an empty list on a statement that takes one parameter is refused everywhere", "a list two elements too long is refused, counted the door's own way"` |
