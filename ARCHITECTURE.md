...
| `bad-input-raises-or-answers` | An argument of the wrong type raises where it is met — a guard in Xqlite or in Xqlite.Pragma, or the native argument decoding inside a raw XqliteNIF stub — while a value of the right type that the library or SQLite refuses comes back as an {:error, reason} answer; the pragma doors judge every argument this way except the connection, which raises. The exception is an argument the library reads as a list (the parameter lists, the cancel-token lists, the authorizer's action list), which is never handed to rustler's decoder and answers {:expected_list, map}, {:expected_keyword_list, map} or {:invalid_cancel_tokens, list_refusal} instead of raising, on the raw stubs as on the Xqlite doors. XqliteNIF's moduledoc states the rule: the wrong TYPE raises (ArgumentError on a raw stub, FunctionClauseError at a guarded Xqlite function), a value of the right type the library refuses answers — a binary that is not UTF-8 where text is meant included, :invalid_utf8_in_string. | `xqlite.ex:@moduledoc`, `xqlite/pragma.ex:@moduledoc` | `bad_input_answers_test.exs: "a guard on an Xqlite function raises", "the native argument decoding raises", "a value of the right type is an answer", "a pragma door raises only for a connection that is not one", "a raw NIF stub raises on a term it cannot decode"` |
